...
before performing this procedure then please see the document in this project
called **Recover_Seagate_Central_Data.md** for detailed instructions on how
to retrieve user Data from a physical hard drive that has been removed from
a Seagate Central.
...
### Optional - Recover any important user Data from the source hard drive
As noted above, this procedure will wipe out all the user data on the
target hard drive. If you wish to save any of the user Data on the target
hard drive then this is the point at which it should be done.
...
is because the Seagate Central will automatically create this partition on system
boot when it detects that it is absent.
...
### Recommended but Optional -  Configure the Seagate Central to grant su / root access   
